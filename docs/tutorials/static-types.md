...
To migrate the `FASTQ` process, rewrite it as follows:
...
To migrate the `QUANT` process, rewrite it as follows:
...
Many {ref}`operators <operator-page>` are not supported in typed workflows. However, an unsupported operator can be replaced by another operator that is supported and/or a standard library function in many cases.
