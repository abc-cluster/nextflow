...
**`Channel<Record> cross( [opts] ) -> Channel<Record>`**

When the `cross` operator is called with named arguments and the source channel is a channel of records, the named arguments are appended to each record in the source channel. Each named argument can be a value or dataflow value.
