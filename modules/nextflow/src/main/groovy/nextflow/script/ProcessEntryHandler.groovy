...
class ProcessEntryHandler {
    ...
    private void assignOutputs(ChannelOut output) {
        ...
        final emitNames = output.getNames()
        if( output.size() == 1 && (emitNames.isEmpty() || emitNames.first() == '$out') ) {
            dsl._publish_('result', output[0])
        }
        else {
            if( output.size() != emitNames.size() )
                log.warn("Process ${processDef.name} is missing emit names for one or more outputs -- unnamed outputs will be omitted")
            for( final name : emitNames )
    ...
    private void publishOutputs() {
        ...
        final outputNames = config instanceof ProcessConfigV1
